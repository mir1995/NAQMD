...
void sh_particle_potential_update(struct Particle *part, struct Potential *pot,
  ...
  if (part->state){
    part->pot_new = pot->func_potup(pot, part->x_new, dim);
    pot->func_gradup(pot, part->pot_grad, part->x_new, dim);
  }
  else{
    part->pot_new = pot->func_potdown(pot, part->x_new, dim);
    pot->func_graddown(pot, part->pot_grad, part->x_new, dim);
  }
  part->rho_new = pot->func_rho(pot, part->x_new, dim);
}
...
void sh_particle_potential_init(struct Particle *particles, struct Potential *pot, 
  ...
  for(unsigned int i=0; i<npart; i++, part++){
    ...
    part->state = true;
    // potential
    part->pot_old = pot->func_potup(pot, part->x_new, dim);
    part->pot_curr = part->pot_old;
    part->pot_new= part->pot_old;
    // gap
    part->rho_old = pot->func_rho(pot, part->x_new, dim);
