...
void odeint_solver_lietrotter(struct Odeint *odeint, struct Particle *ptr, struct Potential *pot){
  for (unsigned int j=0; j<odeint->dim; j++){
    ...
    ptr->x_old[j] = ptr->x_curr[j]; // you'd probably want these two lines more explicitly somewhere else
    ptr->x_curr[j] = ptr->x_new[j]; // you'd probably want these two lines more explicitly somewhere else
    ptr->p[j]     -= odeint->dt / 2 * ptr->pot_grad[j];
    ptr->x_new[j]     += odeint->dt * ptr->p[j];
  }
  double *grad = (double*)calloc(odeint->dim, sizeof(double));
  
  if (ptr->state){
    pot->func_gradup(pot, grad, ptr->x_new, odeint->dim);
  }
  else{
    pot->func_graddown(pot, grad, ptr->x_new, odeint->dim);
  }
  for (unsigned int j=0; j<odeint->dim; j++){
